const translations = {
  en: {
    ...
    experienceValue: "5 years in system design verification",
    languages: "Languages",
    ...
    jobBullets: [
      ...
      "Ran functional, regression, stress, performance, and UI test campaigns across 100-150 devices per run.",
      "Performed failure triage with logs and ADB, then tracked JIRA defects with firmware and validation teams.",
      "Built Python Tkinter tools that reduced per-device setup time from about 7 minutes to about 1 minute.",
  ...
  he: {
    brandSubtitle: "QA | תוכנה | אלקטרוניקה",
    navAbout: "אודות",
    navWork: "ניסיון",
    ...
    location: "מיקום",
    experience: "ניסיון",
    experienceValue: "5 שנות ניסיון ב-System Design Verification",
    languages: "שפות",
    ...
    jobBullets: [
      ...
      "הרצת בדיקות Functional, Regression, Stress, Performance ו-UI על 100-150 מכשירים בכל סבב.",
      "ניתוח ראשוני של תקלות בעזרת לוגים ו-ADB וניהול תקלות ב-JIRA מול צוותי Firmware ו-Validation.",
      "פיתוח כלי Python Tkinter שקיצרו זמן הגדרה למכשיר מכ-7 דקות לכדקה אחת.",
  ...
  ar: {
    brandSubtitle: "QA | برمجة | إلكترونيات",
    navAbout: "نبذة",
    navWork: "خبرة",
    navProjects: "مشاريع",
    navSkills: "مهارات",
    ...
    location: "الموقع",
    experience: "الخبرة",
    experienceValue: "5 سنوات في System Design Verification",
    languages: "اللغات",
    ...
    jobBullets: [
      ...
      "تشغيل اختبارات Functional وRegression وStress وPerformance وUI على 100-150 جهازا في كل دورة.",
      "تحليل أولي للأعطال باستخدام Logs وADB ومتابعة مشاكل JIRA مع فرق Firmware وValidation.",
      "تطوير أدوات Python Tkinter خفضت وقت إعداد الجهاز من حوالي 7 دقائق إلى حوالي دقيقة واحدة.",
...
const skillBars = [
  ["System Verification & Validation", 94],
  ["Python Automation", 88],
  ["Android / ADB Debugging", 86],
  ["Lab Infrastructure", 90],
  ["Electronics Prototyping", 78],
  ["3D Printing & Product Design", 82]
];
...
function applyLanguage(lang) {
  ...
  document.querySelectorAll("[data-i18n-html]").forEach((node) => {
    node.innerHTML = copy[node.dataset.i18nHtml] || "";
  });

  document.querySelectorAll("[data-i18n-list]").forEach((node) => {
    node.innerHTML = "";
    (copy[node.dataset.i18nList] || []).forEach((item) => {
  ...
  storage.set("portfolio-language", lang);
}
...
function renderProjects(filter = "all") {
  ...
  projects
    ...
    .forEach((project) => {
      ...
      const modelMarkup = project.model
        ? `<model-viewer src="${project.model}" camera-controls touch-action="pan-y" auto-rotate ar shadow-intensity="1" aria-label="${project.title} 3D model"></model-viewer>`
        : `<div class="model-placeholder" aria-hidden="true"><span>${project.type === "printing" ? "GLB" : project.type === "electronics" ? "IoT" : "PY"}</span></div>`;
      ...
      article.innerHTML = `
        ...
        <div class="project-body">
          ...
          <a href="${project.link || "#"}" target="${project.link?.startsWith("http") ? "_blank" : "_self"}" rel="noreferrer">Open</a>
        </div>
      `;
      grid.append(article);
    });
...
function renderSkills() {
  const bars = document.querySelector("#skillBars");
  const cloud = document.querySelector("#skillCloud");

  bars.innerHTML = skillBars
    .map(
      ([name, value]) => `
        <div class="skill-bar">
          <div><span>${name}</span><strong>${value}%</strong></div>
          <span class="bar-track"><span style="--value:${value}%"></span></span>
        </div>
      `
    )
    .join("");

  cloud.innerHTML = skillCloud.map((skill) => `<span>${skill}</span>`).join("");
}
...
function setPreference(name, active) {
  html.classList.toggle(name, active);
  storage.set(`portfolio-${name}`, active ? "1" : "0");
}

languageSelect.addEventListener("change", (event) => applyLanguage(event.target.value));
themeToggle.addEventListener("click", () => setTheme(html.dataset.theme === "light" ? "dark" : "light"));
...
accessibilityToggle.addEventListener("click", () => accessibilityPanel.classList.toggle("open"));
accessibilityClose.addEventListener("click", () => accessibilityPanel.classList.remove("open"));
largeTextToggle.addEventListener("change", (event) => setPreference("large-text", event.target.checked));
contrastToggle.addEventListener("change", (event) => setPreference("high-contrast", event.target.checked));
motionToggle.addEventListener("change", (event) => setPreference("reduce-motion", event.target.checked));
...
["large-text", "high-contrast", "reduce-motion"].forEach((name) => {
  const active = storage.get(`portfolio-${name}`, "0") === "1";
  html.classList.toggle(name, active);
});

largeTextToggle.checked = html.classList.contains("large-text");
contrastToggle.checked = html.classList.contains("high-contrast");
motionToggle.checked = html.classList.contains("reduce-motion");

renderProjects();
renderSkills();
applyLanguage(storage.get("portfolio-language", "en"));
